# ============================================================================
# Rakshastra Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://rakshastra-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$RakshastraHome = $(if ($env:RAKSHASTRA_HOME) { $env:RAKSHASTRA_HOME } else { "$env:LOCALAPPDATA\rakshastra" }),
    [string]$InstallDir = $(if ($env:RAKSHASTRA_HOME) { "$env:RAKSHASTRA_HOME\rakshastra-agent" } else { "$env:LOCALAPPDATA\rakshastra\rakshastra-agent" }),
    ...
    # builds apps/desktop into a launchable Rakshastra.exe.
    #
    # Why opt-in:
    #   * Rakshastra-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Rakshastra.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Rakshastra.exe
    ...
    #     `rakshastra desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * Rakshastra Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An autonomous cyber defense agent by Rakshastra.       |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $RakshastraHome)) {
        New-Item -ItemType Directory -Force -Path $RakshastraHome | Out-Null
    }
    $envFile = Join-Path $RakshastraHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $RakshastraHome "node"
...
function Install-Uv {
    # Rakshastra owns its own uv at $RakshastraHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (rakshastra_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `rakshastra update` stay in sync.
    $managedUv = Join-Path $RakshastraHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $RakshastraHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $RakshastraHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $RakshastraHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $RakshastraHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and RAKSHASTRA_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Rakshastra-Setup.exe
...
function Install-Git {
    ...
    which Rakshastra uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\rakshastra\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Rakshastra needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Rakshastra copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\rakshastra\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``RAKSHASTRA_GIT_BASH_PATH`` (User scope) so Rakshastra can find it in a fresh
    ...
    # Download PortableGit into $RakshastraHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $RakshastraHome\git\ ..."
    ...
    try {
        ...
        Write-Info "then re-run this installer.  Rakshastra needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``RAKSHASTRA_GIT_BASH_PATH`` (User env scope) so Rakshastra can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $RakshastraHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $RakshastraHome\git\usr\bin\bash.exe
    $candidates += "$RakshastraHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$RakshastraHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("RAKSHASTRA_GIT_BASH_PATH", $candidate, "User")
            $env:RAKSHASTRA_GIT_BASH_PATH = $candidate
            Write-Info "Set RAKSHASTRA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Rakshastra may not find Git Bash."
    Write-Info "If needed, set RAKSHASTRA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$RakshastraHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$RakshastraHome\node;$env:Path"
        Write-Success "Node.js $version found (Rakshastra-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Rakshastra-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $RakshastraHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $RakshastraHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\rakshastra-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$RakshastraHome\node") { Remove-Item -Recurse -Force "$RakshastraHome\node" }
                Move-Item $extractedDir.FullName "$RakshastraHome\node"
                ...
                $version = & "$RakshastraHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $RakshastraHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\rakshastra-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `rakshastra
                    # update` path (#4735).
                    ...
                    $stashName = "rakshastra-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``rakshastra update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Rakshastra behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running rakshastra processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/rakshastra-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/rakshastra-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/rakshastra-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\rakshastra-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\rakshastra-agent-extract"
    ...
    # next `rakshastra update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Rakshastra-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running rakshastra process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running rakshastra processes before recreating venv..."
            # The launcher CLI (rakshastra.exe) plus its child tree.
            & taskkill /F /T /IM rakshastra.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM rakshastra.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m rakshastra_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not rakshastra.exe.
            ...
            # user (see rakshastra_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `rakshastra.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install rakshastra-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and rakshastra.exe broken with
    ...
    # here, not 30 seconds later when the user runs `rakshastra`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register rakshastra.exe in dist-info/RECORD but fail to
        ...
        # `rakshastra` command while rakshastra-agent.exe / rakshastra-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m rakshastra_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `rakshastra dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `rakshastra dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `rakshastra dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up rakshastra command..."
    
    if ($NoVenv) {
        $rakshastraBin = "$InstallDir"
    } else {
        $rakshastraBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so rakshastra is globally available
    # On Windows, the rakshastra.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$rakshastraBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$rakshastraBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $rakshastraBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set RAKSHASTRA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\rakshastra instead
    # of the Unix default ~/.rakshastra
    $currentRakshastraHome = [Environment]::GetEnvironmentVariable("RAKSHASTRA_HOME", "User")
    if (-not $currentRakshastraHome -or $currentRakshastraHome -ne $RakshastraHome) {
        [Environment]::SetEnvironmentVariable("RAKSHASTRA_HOME", $RakshastraHome, "User")
        Write-Success "Set RAKSHASTRA_HOME=$RakshastraHome"
    }
    $env:RAKSHASTRA_HOME = $RakshastraHome
    
    # Update current session
    $env:Path = "$rakshastraBin;$env:Path"
    
    Write-Success "rakshastra command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.rakshastra-bootstrap-complete which tells the Rakshastra
    ...
    # Rakshastra-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `rakshastra update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".rakshastra-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the RAKSHASTRA_HOME directory structure ($RakshastraHome, default %LOCALAPPDATA%\rakshastra)
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RakshastraHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$RakshastraHome\.env"
    ...
    # Create config.yaml
    $configPath = "$RakshastraHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Rakshastra's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$RakshastraHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in rakshastra_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $RakshastraHome"
    
    # Seed bundled skills into $RakshastraHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $RakshastraHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $RakshastraHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$RakshastraHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $RakshastraHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Rakshastra-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'rakshastra setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\rakshastra-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# the final `electron` -> `Rakshastra` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Rakshastra.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_rakshastra_desktop command
    # resolves apps/desktop/release/win-unpacked/Rakshastra.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Rakshastra-managed
    ...
    # The --dir mode produces an unpacked Rakshastra.exe in
    ...
    # are NOT trying to work around). The Rakshastra icon + product name are
    # stamped onto Rakshastra.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\rakshastra-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Rakshastra.exe",
        "$desktopDir\release\win-arm64-unpacked\Rakshastra.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Rakshastra.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Rakshastra icon + identity are stamped onto Rakshastra.exe by the
    ...
    #    Rakshastra.exe. We deliberately do NOT point them at `rakshastra desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Rakshastra.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Rakshastra.lnk')
        )
        ...
        foreach ($lnkPath in $targets) {
            try {
                ...
                $sc.Description = 'Rakshastra Agent'
                $sc.Save()
        ...
        # with the Rakshastra icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.rakshastra/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'rakshastra setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m rakshastra_cli.main setup
    } else {
        python -m rakshastra_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $rakshastraCmd = "$InstallDir\venv\Scripts\rakshastra.exe"
    if (-not (Test-Path $rakshastraCmd)) {
        $rakshastraCmd = "rakshastra"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$RakshastraHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'rakshastra whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: rakshastra gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$RakshastraHome\logs\gateway.log"
            Start-Process -FilePath $rakshastraCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$RakshastraHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: rakshastra gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: rakshastra gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$RakshastraHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$RakshastraHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RakshastraHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RakshastraHome\rakshastra-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   rakshastra              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   rakshastra setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   rakshastra config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   rakshastra config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   rakshastra gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   rakshastra update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://rakshastra-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
