#!/usr/bin/env node
/**
 * Rakshastra Agent WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.rakshastra/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.rakshastra', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (RAKSHASTRA_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.RAKSHASTRA_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.rakshastra', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.RAKSHASTRA_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.rakshastra', 'document_cache');
const AUDIO_CACHE_DIR = process.env.RAKSHASTRA_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.rakshastra', 'audio_cache');
...
// `rakshastra update` updates bridge.js on disk but a long-lived bridge process
...
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot", "self-chat", or "personal-public"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Rakshastra Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Rakshastra Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot' || WHATSAPP_MODE === 'personal-public') {
          // Check if this is a self-chat even in these modes
          const myNumber = (sock.user?.id || '').replace(/:.*@/, '@').replace(/@.*/, '');
          const myLid = (sock.user?.lid || '').replace(/:.*@/, '@').replace(/@.*/, '');
          const chatNumber = chatId.replace(/@.*/, '');
          const isSelfChat = (myNumber && chatNumber === myNumber) || (myLid && chatNumber === myLid);

          if (isSelfChat && WHATSAPP_MODE === 'personal-public') {
            // Treat self-chat messages as bot queries in personal-public mode
            // fromOwner stays false so it is treated as a user query
          } else {
            // Non-self chat: classify as owner message
      ...
      if (!msg.key.fromMe) {
        ...
        // Personal-Public mode with no allowlist allows everyone.
        // If allowlist is present, only allow those users.
        const hasAllowlist = !!process.env.WHATSAPP_ALLOWED_USERS && process.env.WHATSAPP_ALLOWED_USERS.trim() !== '';
        if (hasAllowlist || WHATSAPP_MODE !== 'personal-public') {
      ...
      // Ignore Rakshastra' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `rakshastra_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
...
if (PAIR_ONLY) {
  ...
  app.listen(PORT, '127.0.0.1', () => {
    ...
    } else if (WHATSAPP_MODE === 'personal-public') {
      console.log(`🔓 Personal-Public mode — no WHATSAPP_ALLOWED_USERS set, allowing everyone.`);
    } else {
      console.log(`🔒 No WHATSAPP_ALLOWED_USERS set — incoming messages are rejected.`);
      console.log(`   Set WHATSAPP_ALLOWED_USERS=<phone> to authorize specific users,`);
      console.log(`   or WHATSAPP_ALLOWED_USERS=* for an explicit open bot.`);
    }
    if ((WHATSAPP_MODE === 'bot' || WHATSAPP_MODE === 'personal-public') && FORWARD_OWNER_MESSAGES) {
