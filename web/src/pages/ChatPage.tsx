/**
 * ChatPage — embeds `rakshastra --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (rakshastra_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    ...
    const gated = !!window.__RAKSHASTRA_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Rakshastra TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[rakshastra-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__RAKSHASTRA_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets RAKSHASTRA_HOME pointed at the
    ...
    // forwarding them into Rakshastra. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="rakshastra-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
