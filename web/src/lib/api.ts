import { buildRakshastraWebSocketUrl } from "@rakshastra/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/rakshastra/). The Python backend
// injects ``window.__RAKSHASTRA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__RAKSHASTRA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const RAKSHASTRA_BASE_PATH = readBasePath();
const BASE = RAKSHASTRA_BASE_PATH;
...
declare global {
  interface Window {
    __RAKSHASTRA_SESSION_TOKEN__?: string;
    __RAKSHASTRA_BASE_PATH__?: string;
    ...
    __RAKSHASTRA_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Rakshastra-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``rakshastra update``, ``rakshastra gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__RAKSHASTRA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__RAKSHASTRA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("rakshastra.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("rakshastra.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__RAKSHASTRA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("rakshastra.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__RAKSHASTRA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Rakshastra dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``rakshastra_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__RAKSHASTRA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Rakshastra-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``rakshastra_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__RAKSHASTRA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildRakshastraWebSocketUrl({
...
export const api = {
  ...
  attackGraphChokepoints: () =>
    fetchJSON<any[]>("/api/v1/attack-graph/chokepoints"),

  // UEBA
  uebaAnomalies: (body: { entity_id?: string; category?: string; severity?: string; since?: string; limit?: number }) =>
  ...
  updateRakshastra: () =>
    fetchJSON<ActionResponse>("/api/rakshastra/update", { method: "POST" }),
  checkRakshastraUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/rakshastra/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** rakshastra-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``rakshastra update``. */
  can_update_rakshastra?: boolean;
  config_path: string;
