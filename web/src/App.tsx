          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="rakshastra-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="rakshastra-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'rakshastra update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "rakshastra update";
    return (
      t.status.updateRakshastraConfirmMessage ??
      `This will run 'rakshastra update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateRakshastraConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateRakshastra) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateRakshastra,
      runningLabel: t.status.updatingRakshastra,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Rakshastra gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateRakshastraConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateRakshastraConfirmTitle ?? `${t.status.updateRakshastra}?`}
    />
    </>
  );
}
