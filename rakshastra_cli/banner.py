...
Pure display functions with no RakshastraCLI state dependency.
"""
...
def _skin_color(key: str, fallback: str) -> str:
    ...
    try:
        from rakshastra_cli.skin_engine import get_active_skin
...
from rakshastra_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

RAKSHASTRA_AGENT_LOGO = """[bold #FFD700]██████╗  █████╗ ██╗  ██╗███████╗██╗  ██╗ █████╗ ███████╗████████╗██████╗  █████╗ [/]
[bold #FFD700]██╔══██╗██╔══██╗██║  ██║██╔════╝██║  ██║██╔══██╗██╔════╝╚══██╔══╝██╔══██╗██╔══██╗[/]
[#FFBF00]██████╔╝███████║███████║███████║███████║███████║███████╗   ██║   ██████╔╝███████║[/]
[#FFBF00]██╔══██╗██╔══██║██╔══██║╚════██║██╔══██║██╔══██║╚════██║   ██║   ██╔══██╗██╔══██║[/]
[#CD7F32]██║  ██║██║  ██║██║  ██║███████║██║  ██║██║  ██║███████║   ██║   ██║  ██║██║  ██║[/]
...
RAKSHASTRA_CADUCEUS = """             [#CD7F32]   ,▅▄▃. .▃▄▅,[/]
             [#CD7F32]  ▅███████████▅[/]
             [#CD7F32] ▐█████████████▌[/]
           [#CD7F32]   ███████████████[/]
           [#CD7F32]  █████████████████[/]
           [#CD7F32]  ████[#FF3333] ▐█████▌[/#CD7F32] ████[/]
           [#CD7F32]  ████[#FF3333] ▐██  █▌[/#CD7F32] ████[/]
           [#CD7F32]  ████[#FF3333] ▐█████◤[/#CD7F32] ████[/]
           [#CD7F32]  ████[#FF3333] ▐██ ◥█▅[/#CD7F32] ████[/]
           [#CD7F32]  ████[#FF3333] ▐██  ◥█[/#CD7F32] ████[/]
...
# (e.g. nix-built rakshastra — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/SKYGOD07/Rakshastra.git"
_OFFICIAL_REPO_CANONICAL = "github.com/skygod07/rakshastra"
...
def _fetch_pypi_latest(package: str = "rakshastra-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Rakshastra update is available.

    Two paths: if ``RAKSHASTRA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = rakshastra_home / ".update_check"
    embedded_rev = os.environ.get("RAKSHASTRA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # RAKSHASTRA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `rakshastra update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/rakshastra/update/check`
    ...
    if embedded_rev:
        ...
        # $RAKSHASTRA_HOME/rakshastra-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = rakshastra_home / "rakshastra-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Rakshastra git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$RAKSHASTRA_HOME/rakshastra-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``rakshastra_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from rakshastra_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/rakshastra-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Rakshastra checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/rakshastra-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Rakshastra Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else RAKSHASTRA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = RAKSHASTRA_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Rakshastra[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Rakshastra[/]")

    if os.getenv("RAKSHASTRA_YOLO_MODE"):
    ...
    try:
        from rakshastra_cli.codex_runtime_switch import get_current_runtime
        from rakshastra_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from rakshastra_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built rakshastra; we know an update
    ...
    # Pip-install warning — `pip install rakshastra-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else RAKSHASTRA_AGENT_LOGO
        console.print(_logo)
        console.print()
