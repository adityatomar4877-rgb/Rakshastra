"""
Status command for rakshastra CLI.

Shows the status of all Rakshastra Agent components.
"""
...
from rakshastra_cli.auth import AuthError, resolve_provider
from rakshastra_cli.colors import Colors, color
from rakshastra_cli.config import get_env_path, get_env_value, get_rakshastra_home, load_config
from rakshastra_cli.models import provider_label
from rakshastra_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from rakshastra_cli.nous_subscription import get_nous_subscription_features
from rakshastra_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``rakshastra config``'s
...
def show_status(args):
    """Show status of all Rakshastra Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Rakshastra Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: rakshastra model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: rakshastra auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: rakshastra auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: rakshastra model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from rakshastra_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from rakshastra_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   rakshastra gateway")
    ...
    jobs_file = get_rakshastra_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_rakshastra_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'rakshastra doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'rakshastra setup' to configure", Colors.DIM))
    print()
