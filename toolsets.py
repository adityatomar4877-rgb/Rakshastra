...
_RAKSHASTRA_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane, and close an agent's
    # read-only terminal tab (both gated on RAKSHASTRA_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (RAKSHASTRA_KANBAN_TASK env set) or the current
...
_RAKSHASTRA_WEBHOOK_SAFE_TOOLS = [
    "web_search",
    "web_extract",
    "vision_analyze",
    "clarify",
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `rakshastra tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``rakshastra tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (RAKSHASTRA_KANBAN_TASK env "
    ...
    # Coding posture (base Rakshastra — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in rakshastra_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Rakshastra toolsets (CLI + messaging platforms)
    #
    ...
    # the `rakshastra send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "rakshastra-cli": {
        ...
        "tools": _RAKSHASTRA_CORE_TOOLS,
        "includes": []
    },

    "rakshastra-cron": {
        # Mirrors rakshastra-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `rakshastra tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as rakshastra-cli; gated by `rakshastra tools`",
        "tools": _RAKSHASTRA_CORE_TOOLS,
        "includes": []
    },
    ...
    "rakshastra-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _RAKSHASTRA_CORE_TOOLS,
        "includes": []
    },

    "rakshastra-homeassistant": {
    ...
    "rakshastra-email": {
        "description": "Email bot toolset - interact with Rakshastra via email (IMAP/SMTP)",
        "tools": _RAKSHASTRA_CORE_TOOLS,
        "includes": []
    },

    "rakshastra-mattermost": {
    ...
    "rakshastra-sms": {
        "description": "SMS bot toolset - interact with Rakshastra via SMS (Twilio)",
        "tools": _RAKSHASTRA_CORE_TOOLS,
        "includes": []
    },
    ...
    "rakshastra-webhook": {
        ...
        "tools": _RAKSHASTRA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "rakshastra-gateway": {
        ...
        "tools": [],
        "includes": ["rakshastra-telegram", "rakshastra-discord", "rakshastra-whatsapp", "rakshastra-slack", "rakshastra-signal", "rakshastra-bluebubbles", "rakshastra-homeassistant", "rakshastra-email", "rakshastra-sms", "rakshastra-mattermost", "rakshastra-matrix", "rakshastra-dingtalk", "rakshastra-feishu", "rakshastra-wecom", "rakshastra-wecom-callback", "rakshastra-weixin", "rakshastra-qqbot", "rakshastra-webhook", "rakshastra-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``rakshastra-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_RAKSHASTRA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``rakshastra-gateway``
    ...
    """
    core = set(_RAKSHASTRA_CORE_TOOLS)
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (rakshastra-<name>).
        # Gives them _RAKSHASTRA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("rakshastra-"):
            platform_name = name[len("rakshastra-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_RAKSHASTRA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
