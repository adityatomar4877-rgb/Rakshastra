...
@pytest.mark.skipif(sys.platform == "win32", reason="requires unix find")
...
@pytest.mark.skipif(sys.platform == "win32", reason="requires unix grep")
...
class TestIgnoreFileWritten:
    ...
    def test_write_index_cache_creates_ignore_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hub_mod, "RAKSHASTRA_HOME", tmp_path)
