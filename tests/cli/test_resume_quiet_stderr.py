...
``tool_progress_mode == "off"`` (set by ``rakshastra chat --quiet``):
...
class TestResumeQuietStderr:
    def test_session_not_found_goes_to_stderr_in_quiet_mode(self, capsys):
        ...
        assert "rakshastra sessions list" in captured.err
